...
use sp1_sdk::{
    include_elf, utils::setup_logger, Elf, HashableKey, ProveRequest, Prover, ProverClient,
    ProvingKey, SP1ProofWithPublicValues, SP1Stdin,
};
use std::fs;
...
pub const PROOF_ADDRESS_ELF: Elf = include_elf!("proof-of-residence-program");
...
/// `examples/proof-of-residence` (parent of this crate).
fn example_dir() -> PathBuf {
    script_dir()
        .parent()
        .expect("proof-of-residence-script should live under examples/proof-of-residence")
        .to_path_buf()
}
...
async fn main() -> Result<()> {
    ...
    let pk = client.setup(PROOF_ADDRESS_ELF).await.unwrap();
    let proof_path = example_dir()
        .join("binaries")
        .join(format!("POR-Attestaion_{}_proof.bin", args.mode));
    let json_path = example_dir()
        .join("json")
        .join(format!("POR-Attestaion_{}_proof.json", args.mode));
